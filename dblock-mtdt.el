  ...
  (let* (
    ...
    (defun bodyContent ()
      ...
      (let* (
        ...
        (setq $mailingName (b:mtdt:header:ofFile:get|mailingname  <mailingFile))
        (when <foldDesc
          (insert (s-lex-format "  [[elisp:(org-cycle)][| /${<foldDesc}/ |]] ")))
        (insert (s-lex-format "  [[elisp:(b:mtdt:compose|basedOnMailingTemplateFile \"${<mailingFile}\")][${$mailingName}]] "  ))
        (when (equalp $type 'originate )
          (insert
           (s-lex-format
            "|| [[elisp:(b:mtdt:compose|basedOnMailingTemplateFile \"${<mailingFile}\")][Originate]] ")))
        ...
        (insert (s-lex-format "|| [[elisp:(b:mtdt:derive/funcUnintern \"${<mailingFile}\")][De-Derive]] "))
        (insert (s-lex-format "|| [[elisp:(b:mtdt:send/selectAndSelMailingToSelRecips \"${<mailingFile}\")][Send with SelRecips]] "))
        (insert (s-lex-format "|| [[elisp:(b:mtdt:mailings|selectWithFile \"${<mailingFile}\")][Select Mailing]]   "))
